...
function doScreenShot (%val)
{
	if (%val)
	{
		...
		while (1)
		{
			%fileName = "screenshot_" @ formatImageNumber ($pref::screenshotNumber += 1);
...
function doPanoramaScreenShot (%val)
{
	if (%val)
	{
		...
		if ($pref::Video::screenShotFormat $= "JPEG")
		{
			panoramaScreenShot ("screenshot_" @ formatImageNumber ($pref::screenshotNumber += 1), "JPEG");
		}
		else if ($pref::Video::screenShotFormat $= "PNG")
		{
			panoramaScreenShot ("screenshot_" @ formatImageNumber ($pref::screenshotNumber += 1), "PNG");
		}
		else 
		{
			panoramaScreenShot ("screenshot_" @ formatImageNumber ($pref::screenshotNumber += 1), "PNG");
