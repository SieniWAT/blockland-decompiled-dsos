...
function handleClientJoin (%msgType, %msgString, %clientName, %clientId, %bl_id, %score, %isAI, %isAdmin, %isSuperAdmin, %trust, %inYourMiniGame)
{
...
function PPUpdate ()
{
	...
	%i = 0;
	while (%i < packColorBox.itemCount)
	{
	...
	%i = 0;
	while (%i < TorsoColorBox.itemCount)
	{
	...
	%i = 0;
	while (%i < accentColorBox.itemCount)
	{
...
function startMissionGui::onWake (%this)
{
	...
	while (%file !$= "")
	{
		...
		{
			SM_missionList.addRow (%i += 1, getMissionDisplayName (%file) @ "\t" @ getMissionPreviewImage (%file) @ "\t" @ %file);
		}
...
function getMissionDescription (%missionFile)
{
	...
	%file.delete ();
	%i = 0;
	while (%MissionInfoObject.desc[%i] !$= "")
	{
		if (%i == 0)
		{
...
function clientCmdClearMapList ()
{
	changeMapList.clear ();
	%i = 0;
	while (changeMapList.description[%i] !$= "")
	{
...
function buildIFLs ()
{
	...
	%fullPath = findFirstFile ("base/data/shapes/player/decals/*.png");
	while (%fullPath !$= "")
	{
		%fileName = fileName (%fullPath);
		%file.writeLine ("decals/" @ %fileName);
		%fullPath = findNextFile ("base/data/shapes/player/decals/*.png");
	}
	%file.close ();
	%file.openForWrite ("base/data/shapes/player/face.ifl");
	%fullPath = findFirstFile ("base/data/shapes/player/faces/*.png");
	while (%fullPath !$= "")
	{
		%fileName = fileName (%fullPath);
		%file.writeLine ("faces/" @ %fileName);
		%fullPath = findNextFile ("base/data/shapes/player/faces/*.png");
	}
	%file.close ();
	%file.delete ();
}
