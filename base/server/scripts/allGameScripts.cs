...
function serverCmdClearColors (%client)
{
	...
	else 
	{
		...
		messageClient (%client, '', %name SPC "null");
	}
}
...
function BanManagerSO::loadBans (%this)
{
	%this.numBans = 0;
	%file = new FileObject ("");
	%file.openForRead ("base/config/server/BANLIST.txt");
	while (!%file.isEOF ())
	{
		%line = %file.readLine ();
		%i = %this.numBans;
...
function ServerCmdClearBricks (%client, %null)
{
	if (%server::Lan)
	{
		return;
...
function ShapeBaseData::Damage (%this, %obj, %position, %null, %amount, %damageType)
{
...
function ProjectileData::onExplode (%this, %obj, %pos)
{
	...
	if (%this.brickExplosionRadius > 0)
	{
		...
		while ((%searchObj = containerSearchNext ()) != 0)
		{
			...
			if (%searchObj.getDataBlock ().indestructable == 1)
			{
				continue;
			}
...
{
	%i = 0;
	while (%i < %removeFromBrick.upSize)
	{
...
{
	%i = 0;
	while (%i < %removeFromBrick.downSize)
	{
...
function killBrick (%brick)
{
	...
	%upCheckval = $currCheckVal += 1;
	%downCheckval = $currCheckVal += 1;
	%killCheckval = $currCheckVal += 1;
	%i = 0;
	while (%i < %brick.upSize)
	{
		%child = %brick.up[%i];
		if (!hasPathToGround (%child, $currCheckVal += 1))
		{
...
function killHangBrick (%checkBrick, %sourceBrick, %iteration)
{
	...
	{
		%i = 0;
		while (%i < %checkBrick.upSize)
		{
			killHangBrick (%checkBrick.up[%i], %checkBrick, %iteration += 1);
			%i += 1;
		}
		if (%iteration <= 1)
		{
...
function fxDTSBrick::setLight (%obj, %data, %client)
{
	...
	if (!$Server::LAN)
	{
		...
		{
			commandToClient (%client, 'CenterPrint', "\c1\c0Players are limited to " @ $Pref::Server::MaxLights_PerPlayer @ " lights each", 2);
			return;
		}
	}
	if ($Server::NumLights >= $Pref::Server::MaxLights_Total)
	{
		commandToClient (%client, 'CenterPrint', "\c1\c0Server is limited to " @ $Pref::Server::MaxLights_Total @ " lights", 2);
		return;
	}
	%light = new fxLight ("")
	{
...
function fxDTSBrick::setEmitter (%obj, %data, %client)
{
	...
	if (!$Server::LAN)
	{
		...
		{
			commandToClient (%client, 'CenterPrint', "\c1\c0Players are limited to " @ $Pref::Server::MaxEmitters_PerPlayer @ " particle emitters each", 2);
			return;
		}
	}
	if ($Server::NumEmitters >= $Pref::Server::MaxEmitters_Total)
	{
		commandToClient (%client, 'CenterPrint', "\c1\c0Server is limited to " @ $Pref::Server::MaxEmitters_Total @ " particle emitters", 2);
		return;
	}
	if (%data.emitterNode !$= "")
	{
	...
	{
		%brickGroup = %obj.getGroup ();
		%brickGroup.numEmitters += 1;
	}
	$Server::NumEmitters += 1;
	%emitter.brick = %obj;
	%obj.emitter = %emitter;
...
function fxDTSBrick::setVehicle (%obj, %data, %client)
{
	...
	if (%data.getClassName () $= "PlayerData")
	{
		...
		if (!$Server::LAN)
		{
			if (%brickGroup.numPlayerVehicles >= $Pref::Server::MaxPlayerVehicles_PerPlayer)
			{
				if (%client.brickGroup == %brickGroup)
				{
					...
					{
						commandToClient (%client, 'CenterPrint', "\c1\c0You already have " @ $Pref::Server::MaxPlayerVehicles_PerPlayer @ " player-vehicles", 2);
					}
				}
				...
				{
					commandToClient (%client, 'CenterPrint', "\c1\c0" @ %brickGroup.name @ " already has " @ $Pref::Server::MaxPlayerVehicles_PerPlayer @ " player-vehicles", 2);
				}
				return;
			}
		}
		if ($Server::numPlayerVehicles >= $Pref::Server::MaxPlayerVehicles_Total)
		{
			...
			{
				commandToClient (%client, 'CenterPrint', "\c1\c0Server is limited to " @ $Pref::Server::MaxPlayerVehicles_Total @ " player-vehicles", 2);
			}
			return;
		}
	}
	else 
	{
		...
		if (!$Server::LAN)
		{
			if (%brickGroup.numPhysVehicles >= $Pref::Server::MaxPhysVehicles_PerPlayer)
			{
				if (%client.brickGroup == %brickGroup)
				{
					...
					{
						commandToClient (%client, 'CenterPrint', "\c1\c0You already have " @ $Pref::Server::MaxPhysVehicles_PerPlayer @ " physics-vehicles", 2);
					}
				}
				...
				{
					commandToClient (%client, 'CenterPrint', "\c1\c0" @ %brickGroup.name @ " already has " @ $Pref::Server::MaxPhysVehicles_PerPlayer @ " physics-vehicles", 2);
				}
				return;
			}
		}
		if ($Server::numPhysVehicles >= $Pref::Server::MaxPhysVehicles_Total)
		{
			...
			{
				commandToClient (%client, 'CenterPrint', "\c1\c0Server is limited to " @ $Pref::Server::MaxPhysVehicles_Total @ " physics-vehicles", 2);
			}
			return;
		}
	}
...
function Vehicle::activate (%vehicle, %null, %activatingClient, %pos, %vec)
{
...
function ServerCmdClearAllBricks (%client)
{
	...
	{
		MessageAll ('MsgClearBricks', "\c3" @ %client.name @ "\c1\c0 cleared all bricks.");
	}
	%x = 0;
	while (1)
	{
...
function TrustListCheck (%obj1, %obj2, %null)
{
...
function InteractionCheck (%strType, %obj0, %obj1, %obj2, %obj3)
{
	...
	{
		%i = 0;
		while (%i < $Interaction::NumObjects[%strType])
		{
			eval ("%testObj = %obj" @ %i @ ";");
			if (isObject (%testObj))
			{
